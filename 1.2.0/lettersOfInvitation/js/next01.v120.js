    ...
    $.changeAjax({
        ...
        callback: function (res) {
            ...
            if (data.organization_name) {
                ...
                $(".friendName").attr("data-id", data.push_object_id);
                //修改顶部标题
    ...
    if (validate(ortype)) {
        if (ortype == 2) {
            $.changeAjax({
                ...
                callback: function (res) {
                    ...
                    if (data.user_name) {
                        $("#realName").val(data.user_name);
                        $("#realName").attr("disabled", true);
                    } else {
                        $("#realName").attr("disabled", false);
                    }
                    if (data.user_id) {
                        $("#realName").attr("data-id", data.user_id);
                    }
                    if (data.user_sex) {
                        if (data.user_sex == "男") {
        ...
        if (ortype == 3) {
            $(".nextDiv").on("click", function () {
                ...
                console.log(realName + "," + realSex + "," + businessType);
                ...
                $.changeAjax({
                    ...
                    data: {
                        ...
                        pushObjectId: $(".friendName").attr("data-id"),
                        userPhone: newPhone,
                        zoneDescriptionId: newZoneId,
                            ...
                            if (data.examineState && data.examineState == 1) {
                                ...
                                if (invitationCode == "") {
                                    if (JSON.stringify(data) == "{}") {
                                        console.log("空对象");
                                        window.location.href = "./haveApplied.html?name=" + baseName + "&inviterId=" + inviterId + "&np=" + np + "&nz=" + nz + "&myOrganizationId=" + myOrganizationId + "&ty=3";
                                    ...
                                    if (JSON.stringify(data) == "{}") {
                                        console.log("空对象");
                                        window.location.href = "./next02.html?name=" + baseName + "&inviterId=" + inviterId + "&np=" + np + "&nz=" + nz + "&myOrganizationId=" + myOrganizationId + "&ty=3";
                                    } else if (validate(data.invitationCodeType)) {
            ...
            $(".nextDiv").on("click", function () {
                var toporganizationName = $(".friendName").text();
                console.log($("#codeNum").val() + "," + myOrganizationId + "," + $("#realName").attr("data-id") + "," + $(".friendName").attr("data-id") + "," + newPhone + "," + newZoneId + "," + toporganizationName);
                $.changeAjax({
                    url: "myOrganizationApi/invitationJoinOrganization.do",
                    data: {
                        organizationType: 2,
                    ...
                    callback: function (res) {
                        ...
                        var base = new Base64();
                        var baseName = base.encode(urlName).replace(/\=/g, "*");
                        var invitationCode = $("#codeNum").val();
                        var data = res.data;
                        if (data) {
                            if (data.examineState && data.examineState == 1) {
                                ...
                                if (invitationCode == "") {
                                    ...
                                    if (JSON.stringify(data) == "{}") {
                                        console.log("空对象");
                                        window.location.href = "./haveApplied.html?name=" + baseName + "&inviterId=" + inviterId + "&np=" + np + "&nz=" + nz + "&myOrganizationId=" + myOrganizationId + "ty=2";
                                    } else if (validate(data.organizationType)) {
                                        if (data.organizationType == 4) {
                                            window.location.href = "./haveApplied.html?name=" + baseName + "&inviterId=" + inviterId + "&np=" + np + "&nz=" + nz + "&myOrganizationId=" + myOrganizationId + "&ty=2";
                                        } else if (data.organizationType == 1) {
                                    ...
                                    if (JSON.stringify(data) == "{}") {
                                        console.log("空对象");
                                        window.location.href = "./next02.html?name=" + baseName + "&inviterId=" + inviterId + "&np=" + np + "&nz=" + nz + "&myOrganizationId=" + myOrganizationId + "&ty=2";
                                    } else if (validate(data.invitationCodeType)) {
                                        if (data.invitationCodeType == 2) {
                                            $.MessagePrompt({
                                                text: '邀请码错误！',
